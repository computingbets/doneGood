...
*/
urls = [
  'http://melanzana.com/catalog/index.php/cPath/22'
]
...
.then(function(pages){
	...
	pages.forEach(function($){
	  ...
	  $('.productListing-data').each(function(index, elem){
...
.then(function(pageUrls){
	...
	.then(function(responses){
		...
		responses.forEach(function(response){
		  ...
		  productName = $('h1').text();
      //$('p.price > span').text();
		  productDescription = $('#fun').text();
      prodDescription = productDescription.split("$")
      console.log(prodDescription);
      productPrice = prodDescription[1];
      pageTitle = $('title').text();
		  imageUrl = $('.slick-slide').eq(4).attr("src");
