...
.then(function(pages){
  ...
  pages.forEach(function($){
    ...
    $('.grid-view-item__link grid-view-item__image-container').each(function(index, elem){
      ...
      var productUrl = https + $(elem).attr("href");
      ...
      var imageUrl = $(elem).find('.grid-view-item__image.lazyautosizes.lazyloaded').attr('src').slice(0, 2);
      var productObj = {
        productUrl: productUrl,
        imageUrl: imageUrl
      };
      console.log(productUrl, imageUrl);
